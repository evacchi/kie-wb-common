...
public class BpmnMatch<In, Out> {

    private final Class<?> outputType;
    LinkedList<BpmnMatch.Case<?, Out>> cases = new LinkedList<>();
    Function<In, Out> orElse;

    public BpmnMatch(Class<?> outputType) {
        this.outputType = outputType;
    }

    public static <In, Out> BpmnMatch<In, Out> of(Class<In> inputType, Class<Out> outputType) {
        return new BpmnMatch<>(outputType);
    }

    public static <In, Out> BpmnMatch<In, Node<? extends View<? extends Out>, ?>> ofNode(Class<In> inputType, Class<Out> outputType) {
        return new BpmnMatch<>(outputType);
    }

    public static <In, Out> BpmnMatch<Node<? extends View<? extends In>, ?>, Out> fromNode(Class<In> inputType, Class<Out> outputType) {
        return new BpmnMatch<>(outputType);
    }

    public static <In, Out> BpmnMatch<In, Edge<? extends View<? extends Out>, ?>> ofEdge(Class<In> inputType, Class<Out> outputType) {
        return new BpmnMatch<>(outputType);
    }
    ...
    public <Sub> BpmnMatch<In, Out> when(Class<Sub> type, Function<Sub, Out> then) {
        Function<Sub, Result<Out>> thenWrapped = sub -> Result.of(then.apply(sub));
        return when_(type, thenWrapped);
    }

    public <Sub> BpmnMatch<In, Out> when_(Class<Sub> type, Function<Sub, Result<Out>> then) {
     ...
     */
    public <Sub> BpmnMatch<In, Out> missing(Class<Sub> type) {
        return when_(type, reportMissing(type));
    }

    public <Sub> BpmnMatch<In, Out> ignore(Class<Sub> type) {
        return when_(type, ignored(type));
    }

    public BpmnMatch<In, Out> orElse(Function<In, Out> then) {
        this.orElse = then;
        return this;
    }
